 ...
 * all-or-nothing linkedDirs validation — silently dropping the reference the
 * user picked. External / imported roots (an absolute `metadata.baseDir`) are
 * the user's own folders and are never created here.
 */
export async function ensureReferencedProjectDir(
  ...
  // Best-effort: a failure here just leaves resolvedDir pointing at the
  // computed (not-yet-created) path, i.e. the prior behavior.
  await ensureProject(projectsRoot, project.id, project.metadata).catch(() => {});
}
