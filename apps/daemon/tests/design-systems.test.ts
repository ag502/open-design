...
describe('design systems registry', () => {
  ...
  it('does not seed or backfill agent-managed review artifacts', async () => {
    const created = await createUserDesignSystem(root, {
      title: 'Agent Managed',
    ...
    await writeFile(
      path.join(root, created.id.slice('user:'.length), 'README.md'),
      '# Agent Managed\n\nGenerated by the project agent.\n',
      'utf8',
    );

    const generatedFiles = await listUserDesignSystemFiles(root, created.id);

    expect(generatedFiles?.map((file) => file.path)).toEqual(['DESIGN.md', 'README.md']);
  });
});
