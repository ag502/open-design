...
type SetupStep = 'setup' | 'confirm' | 'generating';
...
function rememberGenerationJob(designSystemId: string, jobId: string): void {
  try {
    window.sessionStorage.setItem(generationJobStorageKey(designSystemId), jobId);
  } catch {
...
export function DesignSystemCreationFlow({
  ...
  useEffect(() => {
    if (step !== 'generating' || !job?.id || handedOffJobId === job.id) return undefined;
    const jobId = job.id;
    let cancelled = false;
    ...
    async function poll() {
      ...
      if (cancelled) return;
      if (!next) {
        setError('Could not read generation progress.');
        setStep('setup');
        return;
      }
      setJob(next);
      if (next.designSystemId) {
        rememberGenerationJob(next.designSystemId, next.id);
        setHandedOffJobId(next.id);
        await onSystemsRefresh?.();
        if (cancelled) return;
        onCreated(next.designSystemId);
        return;
      }
      if (next.status === 'failed') {
        setError(next.error ?? 'Could not generate this design system.');
        setStep('setup');
        return;
      }
  ...
  }, [handedOffJobId, job?.id, onCreated, onSystemsRefresh, step]);
    ...
    return (
      <div className="ds-workspace">
        ...
        <main className="ds-generation-stage">
          ...
          <p>{job?.message ?? 'Keep this tab open and come back in a few minutes'}</p>
          <div
            className="ds-progress-line"
...
function buildSourceContextManifest(
  ...
  sections.push(
    ...
    '- SKILL.md should include YAML frontmatter with `name`, `description`, and `user-invocable`, plus usage guidance that points agents at README.md, DESIGN.md, colors_and_type.css, preview/, assets/, build/, fonts/, source_examples/, and ui_kits/app/.',
    ...
    '- source_examples/ or equivalent root/nested source files should preserve selected high-signal original components when snapshots include substantial app/component source, similar to Claude Design exports that keep files like SelectModelButton.tsx or ChatNavBar/index.tsx alongside the package.',
    '- ui_kits/app/ should contain an applied interface example, plus substantive role-based files under `ui_kits/app/components/` when the source snapshots include representative app shells, navigation, chat/input surfaces, or reusable components. `ui_kits/app/index.html` must load `../../colors_and_type.css`, must load/import/compose the modular component files, and must mount/render the composed interface instead of staying as a standalone generic static mock or disconnected script list. If the entry directly loads `.jsx`/`.tsx` files, include React, ReactDOM, and Babel standalone scripts and expose each loaded component as `window.ComponentName` / `globalThis.ComponentName`, or write compiled browser-ready JavaScript instead. For chat/workspace evidence, cover app shell, sidebar/navigation, assistant/list rail, chat area, input bar/composer, and message bubble/comment roles; the app shell component must compose those roles into one product-like surface. Placeholder component shells are not sufficient.',
