---
name: atom-design
zh_name: "Atom · 抗 AI 套路设计技能"
en_name: "Atom · Anti-AI-slop Design Skill"
emoji: "⚛️"
description: "Anti-AI-slop design skill for greenfield pages, audits, redesigns, and design extraction from URLs or screenshots. 22-theme catalog · 21 macrostructures · 46 component archetypes · 65-gate slop test · project memory. Use when the user asks to build a new app or landing page, wants to redesign something, invokes Atom by name, or uses audit/redesign/study."
version: 1.0.0
...
featured: 1
recommended: 1
tags: ["design-skill", "anti-slop", "landing-page", "hallmark", "theme-catalog", "macrostructure", "slop-test", "study"]
triggers:
  - "atom"
  - "atom-design"
  - "atom audit"
  ...
  - "anti slop"
  - "hallmark"
example_id: sample-atom-design
example_name: "Atom · 设计流程主页"
example_format: html
example_tagline: "22 themes · 21 macrostructures · 65-gate slop test"
example_desc: "Atom 设计技能的 catalog 主页, 22 个主题切换 + 5 个 named landing page 展示位"
example_source_url: "https://github.com/Nutlope/hallmark"
example_source_label: "Nutlope/hallmark (MIT)"
od:
  mode: prototype
  surface: web
  scenario: design
  ...
  design_system:
    requires: false
  example_prompt: "用「Atom · 抗 AI 套路设计技能」做一个 landing page. 先按 atom 的流程: pre-flight scan → genre 检测 → macrostructure pick → 22 主题选 1 → preview block → build → 65 gate slop test. 不要重复上一次的 macrostructure + theme + accent。"
---
...
Atom is opinionated, short, and boring on purpose. It encodes a tight set of rules — drawn from the consensus of the anti-AI-slop design field (impeccable, kami, Anthropic's frontend-design skill, taste-skill, the Claude cookbook on frontend aesthetics, and the 2026 "tactile rebellion" movement) — and refuses to let the model fall back to the defaults every LLM was trained on.

The differentiator: Atom insists on **structural variety**, not just visual variety. Two pages by Atom for two different briefs should not share the same hero → 3-feature → CTA → footer rhythm. They should feel like different sites, not different colour-swaps of the same template. See [`references/structure.md`](references/structure.md).

Adapted from Nutlope/hallmark (MIT). The discipline, rules, and 22-theme catalog are theirs; the integration with Open Design's `.od/` runtime and verb plumbing is ours.

---

## How to use this skill

Atom has one default behaviour and three explicit verbs.

| Invocation | What it does |
| --- | --- |
| *(default)* | The user asked you to design or build something new. Follow the **Design flow** below. |
| `atom audit <target>` | Read the target, score it against the anti-pattern list, return a ranked punch list. **Do not edit.** |
| `atom redesign <target> [--mood <name>]` | Take the target's content and intent, then redesign the visual structure **inside the existing implementation boundaries unless the user explicitly confirms a full rebuild.** New section rhythm, new heading placement, new component voice. Preserve existing routes, component ownership, copy intent, brand, and information architecture; replace only the visual/interaction layer needed for the requested scope. |
| `atom study <screenshot \| URL>` | The user pasted or attached an image of a design they admire, **or** pasted a URL to a live page. Extract the **DNA** — macrostructure, archetypes, type-pairing, colour anchor — and produce a diagnosis report, then optionally rebuild the user's content using the extracted DNA **or** emit a portable `design.md` of the DNA. Detection is automatic: a URL (`http://` / `https://` prefix) routes to URL mode; anything else routes to image mode. **URL mode** reads the page's HTML and CSS via WebFetch — it can name exact fonts and exact colour values, but can't judge rhythm. After the diagnosis, the user has three follow-ups: build with the DNA (handoff to default), lock the DNA into a portable `design.md` (opt-in via "lock the DNA" / "give me a design.md"), or stop at the diagnosis. **Never copies pixels. Refuses template-marketplace URLs and competitor pages. Tighter refusal layer for `design.md` emission than for the diagnosis itself — URL-mode emission requires attestation that the source is the user's own or a public reference for their own brand. Falls back to asking for a screenshot if the URL is auth-walled, a JS-only SPA shell, or otherwise un-readable.** Load [`references/study.md`](references/study.md) before this verb runs. |
...
**Implementation safety rail.** Atom is a design skill, not a license to bulldoze a codebase. In any existing project:
...
1. **Pre-emit self-critique.** Before handing back any output, score it 1–5 on six axes — Philosophy, Hierarchy, Execution, Specificity, Restraint, Variety. Anything **< 3** triggers a revision pass. Stamp the six scores at the top of the artifact (`/* Atom · pre-emit critique: P5 H4 E5 S4 R5 V5 */`). See [`references/slop-test.md`](references/slop-test.md) § Pre-emit self-critique.
...
- **Slop test — universal-only subset.** Run the visual / microinteraction / contrast (gates 46–50) / a11y / typography gates. Skip the diversification gates (no `.od/atom/log.json` entry — components don't rotate) and skip the layout-safety gates that assume a full page.
...
- **Project-memory append.** No `.od/atom/log.json` entry for component runs. The diversification rule doesn't apply.
...
1. **The component artifact** — a single self-contained file matching the project's conventions:
   ...
   - The component consumes Atom tokens by name (`var(--color-accent)`), never inlines OKLCH values.
...
```css
/* Atom · component: <type> · genre: <genre> · theme: <theme>
 * states: default · hover · focus · active · disabled · loading · error · success
 * contrast: pass (46–50)
 */
```

The `component:` prefix tells future Atom runs this artifact is component-scoped and shouldn't trigger page-level diversification rules. The `states:` line is a checklist — every state listed must have actual styling in the file.
...
If the project already has code — a `package.json`, a `tailwind.config.*`, an `index.html`, any CSS — Atom should **read it before asking the user anything**. Stomping on an established palette or font stack is the difference between a skill the user keeps and a skill the user uninstalls.

**Six signal sources, scanned in order:**

0. **`design.md`** — at the project root (or `DESIGN.md`). If present, this is the **locked design system for the project** — written by a previous `atom redesign` run on the whole app, or by hand. **Read it first; it overrides everything else.** Subsequent picks (genre, theme, type, motion) defer to it. The diversification rule is *inverted* on `design.md`-managed projects: pages must share the system, not differ from each other. See [`verbs/redesign.md`](references/verbs/redesign.md) § Multi-page flow for how the file is produced and amended.
...
Atom will preserve: font stack, palette, spacing scale.
Atom will introduce: macrostructure, microinteraction discipline,
slop-test gates, hero enrichment recipe.

If you want Atom to override any preserved item, say so.
```

**Persistence.** Write the findings to `.od/atom/preflight.json` once. On subsequent runs, *re-use* the cached findings unless either:
...
- **No signals found** (vanilla HTML project, empty repo, scratch directory) → silent. One line only: *"No pre-flight signals — proceeding with full Atom stack."*
...
> *Pre-flight findings: vanilla HTML, no framework detected. No motion library, no Tailwind, no design tokens. Atom will introduce: full token system, macrostructure, microinteraction discipline, slop-test gates. Nothing to preserve.*
...
> *Atom will preserve: Tailwind tokens, the `tokens.json` file (won't overwrite). Atom will introduce: macrostructure, microinteraction discipline, slop-test gates. Motion stance: motion-cut (no framer-motion / motion / gsap detected).*
...
Atom works best when you know three things before writing code:
...
**Always ask — answering is optional.** Atom **always** asks before it designs. The bundled question is the first thing the user sees after the pre-flight block. Even on a five-word brief — *"design a podcast site"*, *"build a SaaS landing"*, *"make me a portfolio"* — ask. Especially on those briefs, since they're where the model is most tempted to invent.
...
1. Look in the target codebase for an existing `/* Atom · macrostructure: <name> · ... */` stamp at the top of any CSS file. If you find one, your pick must be a *different* macrostructure.
2. If you have produced any other Atom output for this user in this session, your pick must be a different macrostructure than the last one.
...
**Theme-diversification rule (mandatory).** Picking a different macrostructure isn't enough on its own — two consecutive Atom outputs can share a theme even if their structures differ, and the result reads as repetition. Two consecutive themes must differ on **at least one** of three axes:
...
If the file doesn't exist, this is the first Atom run for this project — no constraint, but **you'll create the file in Step 6**.
...
- **First-time** (no `log.json`, fresh project): no rotation block at all — just the macrostructure pick. *"This is the first Atom run for this project. Picking Long Document — fits the Coffeebox brief's editorial tone."*
...
- [`design-md.md`](references/design-md.md) — load only when the user explicitly asks Atom to lock the system into a portable file (phrases: *"lock the system"*, *"give me a design.md"*, *"make this portable"*, etc.). Opt-in; never fires on a vanilla build.
...
- [`study.md`](references/study.md) — load only when `atom study` runs.
...
7. **Diversification** *(optional, only when `.od/atom/log.json` has prior entries)* — what axes differ vs the previous run.
...
- **Multi-format exports on `design.md` projects.** If a `design.md` exists at the project root (a system-managed project), append all four export formats — `tokens.css`, Tailwind v4 `@theme`, DTCG `tokens.json`, shadcn/ui CSS variables — into `design.md`'s `## Exports` section. Load [`export-formats.md`](references/export-formats.md) for the canonical mapping from Atom tokens to each format. Single-page projects skip this step (they get only `tokens.css`).
- **Opt-in `design.md` (lock-the-system flow).** If the user explicitly asks Atom to lock the build's design system into a portable file (phrases: *"lock the system"*, *"give me a design.md"*, *"make this portable"*, etc.), load [`design-md.md`](references/design-md.md) and follow it. Page-scope only; component-scope skips. **The default verb does NOT auto-emit `design.md`** — users iterate freely first, then ask for it once the system is settled. If `design.md` already exists, refresh its `## Exports` section instead of overwriting. The Step 5 preview block carries a one-line CTA surfacing this option after every page-build.
...
```css
/* Atom · macrostructure: Marquee Hero · H1 hero knobs: size=xxl, alignment=left-bias
...
If `references/study.md` cannot be loaded for any reason, refuse the verb politely and direct the user to `atom redesign` with a written description of what they want from the source.
