# `atom audit input.html`

> User invocation: *"atom audit input.html"*
...
Do **not** run `hallmark refine` here. Run `atom redesign` instead — `refine` preserves structure, and the structure itself is the primary problem. See [`../redesign/`](../redesign/) for what a redesign of a similar page looks like.
