...
A small set of pre-generated abstract / decorative imagery that any Atom output can pull from when a brief allows non-photographic imagery. The kit lives at:
...
**Post-processing.** Trim, transparent-background where applicable, colour-balance against Atom's OKLCH palette tokens, save as WebP for size + PNG where alpha matters. Ornaments as inline SVG so they inherit `currentColor`.
