...
Loaded by [`SKILL.md`](../SKILL.md) Step 6 when emitting the design system as portable tokens. Defines the four canonical formats Atom always writes:
...
## Token taxonomy — Atom's source of truth

Every Atom output writes these tokens (or a subset, if the page doesn't use one). The names are the source; every other format is a translation.

| Atom token | Type | Example value |
| --- | --- | --- |
...
The source. Plain CSS custom properties at `:root`. Every Atom page CSS imports this file at the top:
...
Tailwind v4 reads CSS variables inside `@theme` and generates utilities (`bg-paper`, `text-ink`, etc.) automatically. The translation from Atom tokens is direct — same variable names, same OKLCH values:
...
@theme {
  ...
  /* Spacing — Tailwind reads --spacing-* by default; we keep Atom's --space-* names alongside */
  --spacing-3xs: 0.25rem;
  --spacing-2xs: 0.5rem;
  --spacing-xs:  0.75rem;
  --spacing-sm:  1rem;
...
Notes:
- Tailwind v4 expects `--spacing-*` (with `ing`) for the spacing utilities. We mirror Atom's `--space-*` to `--spacing-*` so both names work.
...
The translation from Atom tokens:
...
:root {
  ...
  --accent:               58%   0.16  60;     /* accent (same as primary in Atom) */
...
Then the user's `tailwind.config.ts` (or v4 `@theme`) reads them with `oklch(var(--background))`. Atom's accent → shadcn's primary is the canonical mapping; secondary → paper-3.

If the user wants a dark variant, mirror with the dark theme tokens under a `.dark` selector (Atom's Midnight / Bloom / Terminal supply the values).
