...
**Diversification rule:** within a single page, no two sections should use the same archetype. A Bento Grid might pair *Bento feature block* with *Inline form CTA* with *Logo wall (hairline)*. The next page Atom builds should pick different archetypes from the same categories.
...
```css
/* Atom · macrostructure: Bento Grid · F1 Bento knobs: tiles=6, spans=irregular, accent=corner-only · ... */
```
...
**Diversification.** Same rule as nav — across consecutive Atom runs in the same session, no two outputs should share the same footer archetype.
...
**Diversification.** Across consecutive Atom runs in the same project session, no two outputs should share the same nav archetype — even when they share a genre. If the previous run used N5 on a modern-minimal page, the next modern-minimal page picks N1 or N9 from the routing table's "also" column.
...
The goal is composed variety — within a page, sections feel different from each other; across pages Atom builds, sections feel different from the last.
