...
describe("packaged smoke workflow", () => {
  ...
  it("keeps the self-hosted beta lane metadata-driven with reusable platform publish scripts", async () => {
    ...
    expect(workflow).toContain("Upload win_x64 publish manifest");
    expect(workflow).toContain("open-design-beta-s-win-x64-publish-manifest");
    ...
    expect(workflow).toContain("Upload mac_arm64 publish manifest");
    expect(workflow).toContain("open-design-beta-s-mac-arm64-publish-manifest");
  ...
  it("keeps beta runner bootstrap in workflows instead of release scripts", async () => {
    ...
    expect(workflow).toContain("npm install --global pnpm@10.33.2");
    expect(workflow).toContain("pnpm install --frozen-lockfile");
    expect(workflow).not.toContain("PATH: /usr/local/libexec/open-design/wrappers:${{ env.PATH }}");
    expect(workflow).not.toContain("clean: false");
